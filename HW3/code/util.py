...
# "learner."
def loadTransProb() -> dict[tuple[tuple[int, int], tuple[int, int]], float]:
    ...
    raise Exception(
        "could not load " + transFilePath + ". Did you run learner on this layout?"
    )
...
class Belief(object):
    ...
    def setProb(self, row: int, col: int, p: float):
        self.grid[row][col] = p
    ...
    def addProb(self, row: int, col: int, delta: float):
    ...
    def getProb(self, row: int, col: int):
