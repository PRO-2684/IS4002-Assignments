...
class UtilGobang:
    ...
    def visualize_board(self, random_response):
        ...
        text = (
            "Black wins."
            if color == 1
            else "White wins." if color == 2 else "Tie." if color == 0 else None
        )
        if text is not None:
    ...
    def count_max_connections(self, state) -> Tuple[int, int]:
        return (
            self.count_max_connections_for_single_color(state, 1),
    ...
    def update_board(
        self, random_response, learning: bool = True, attempt: int = 8
    ) -> Tuple[int, bool]:
        action_space = copy.deepcopy(self.action_space)
        (
            next_state_free_of_response,
            next_state,
            current_black_connection,
            current_white_connection,
            next_black_connection,
            next_white_connection,
            reward,
        ...
        return (
            ...
            if next_black_connection >= self.bound
                ...
                (2, True)
                if next_white_connection >= self.bound
                else (0, True) if len(self.action_space) == 0 else (-1, False)
            )
        )

    def evaluate_agent_performance(
        self, random_response, model, opponent, episodes=1000
    ):
        opponent_name = "random noise" if random_response else "training model itself"
        ...
        for _ in tqdm(range(episodes)):
            ...
            while True:
                ...
                black_wins, white_wins, ties = (
                    (black_wins, white_wins, ties)
                    if end_up_gaming is False
                    else (
                        (black_wins, white_wins, ties + 1)
                        if color == 0
                        else (
                            (black_wins + 1, white_wins, ties)
                            if color == 1
                            else (black_wins, white_wins + 1, ties)
                        )
                    )
                )
                if end_up_gaming:
...
class Gobang(UtilGobang):
    ...
    def get_next_state(
        self, action: Tuple[int, int, int], response: Tuple[int, int, int]
    ) -> np.array:
        black, xb, yb = action
    ...
    def sample_action_and_response(
...
def _sample_action_and_response(chessboard, actor, state):
    ...
    response = (
        None
        if len(np.nonzero(state == 0)[0]) <= 1
        else _sample_response(chessboard, actor, x, y)
    )
...
def train_model(model, num_episodes=1000, checkpoint=1000, gamma=0.5):
    ...
    for _ in range(num_episodes):
        ...
        for count in range(chess_board.board_size**2 // 2 + 1):
            ...
            black_1, white_1, black_2, white_2, reward = (
                chess_board.get_connection_and_reward(action=action, response=response)
            )
            ...
            stop = (
                ...
                if (
                    black_2 >= model.bound
                    or white_2 >= model.bound
                    or len(np.nonzero(next_state == 0)[0]) == 0
                )
                else False
            )
        ...
        entropy = -float(
            torch.mean(torch.sum(policy * torch.log(policy + 1e-6), dim=1))
        )
